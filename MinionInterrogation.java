...
class Minion implements Comparable<Minion>{
	...
	Minion(int number, int time, int numerator, int denominator){
		_number=number;
		double chance=(double)numerator/(double)denominator;
		_expected=time/chance;
	}
		
	@Override
	public int compareTo(Minion o) {

		if (o._expected > _expected) {		
			return -1;
		}
		else if (o._expected == _expected) {
			return 0;
		}
		else
		{
...
public class MinionInterrogation {
	...
	public static void main(String[] args) {
		...
		int[][] minions = {{390, 185, 624}, {686, 351, 947}, {276, 1023, 1024}, {199, 148, 250}};
		int[][] minions2 = {{5, 1, 5}, {10, 1, 2}};
		
		answer(minions);
	}
		
	static int[] answer(int[][] inputMinions){
				
		Minion[] minions = new Minion[inputMinions.length];
		
		for (int x=0; x<inputMinions.length; x++){
			minions[x] = new Minion(x, inputMinions[x][0],inputMinions[x][1],inputMinions[x][2]);
		}
		
		Arrays.sort(minions);
		
		int[] returnValue = new int[minions.length];
		
		for (int x=0; x<minions.length; x++){
			returnValue[x]=minions[x]._number;
		}
